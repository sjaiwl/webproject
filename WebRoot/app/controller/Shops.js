...
Ext.define('MyApp.controller.Shops', {
    ...
    add: function(target) {
        ...
        var formWindow = Ext.create('widget.shopform'),	// Create new form window
        	form = formWindow.down('form').getForm(),		// Get form within window
        	model = Ext.create('model.shop');				// Create new Record model
    ...
    edit: function(target) {
        ...
        var data = target.up('panel').data,						// Get panel's assosiated data
        	store = this.getShopsStore(),						// Get Records store
        	record = store.getById(data.id),					// Get current record
        	formWindow = Ext.create('widget.shopform'),		// Create new form window
        	form = formWindow.down('form').getForm();			// Get form within window
    ...
    remove: function(target) {
        ...
        // Confirm this delete
        Ext.Msg.confirm('确定', '你确定要删除记录吗？', function(btn) {

            // User confirmed yes
            if (btn == 'yes') {

                var data = target.up('panel').data,				// Get assosiated data
                    store = me.getShopsStore(),				// Get Records store
                    //record = store.getById(data.id),			// Get current record
